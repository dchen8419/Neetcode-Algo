...
When we work with arrays, we technically have two sets of values available: we have the values in the array (in this case, just integers), 
and we can work with the indices of the array itself.

So take a five-element array: [1, 4, 5, 3, 3]

We have the numbers 1, 4, 5, 3, 3 in the values itself. But we also have the numbers 0, 1, 2, 3, 4 as indices of the array. 
If you add one to each of those indices, we would have 1, 2, 3, 4, 5. Those are the numbers we're looking for in an ideal scenario.

So what we can do is use those indices and "flag" them in some way to say "Hey, we saw this index in the values of the array". 
To do that, we look at each actual value (1, 4, 5, 3, and 3). Then we subtract one from it to get its corresponding array index. 
Then we modify the array to flag it. We multiply the value at that index by -1 to make it negative.

You'll see in the code that we have to be careful, if we've already visited an index, it will be negative, 
and when we subtract 1 to find the index, we'll have a problem. So we actually take the absolute value. Don't worry if this is hard to follow, 
there's a comment in the code about it.

Then we loop through the array again and build a result. 
We check each index (again, we don't care about the actual absolute value of the number there, we're just looking for our flag - the negative sign). 
